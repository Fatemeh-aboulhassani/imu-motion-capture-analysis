...
MTW_IDS_ALLOWLIST = list(SENSOR_LOCATION_MAP.keys())
...
def make_outlet(device_id_str: str, sensor_location: str):
    labels = [
        "qw","qx","qy","qz",
        "ax","ay","az",
        "gx","gy","gz",
    ...
    info = StreamInfo(
        name=f"{LSL_PREFIX}_{sensor_location}",
        type=LSL_TYPE,
        channel_count=len(labels),
        nominal_srate=float(TARGET_RATE_HZ),   # what we *intend* to run at
        channel_format="float32",
        source_id=f"{LSL_PREFIX}_{sensor_location}",
    )
    ...
    try:
        while True:
            ...
            if item is not None:
                ...
                sensor_location = SENSOR_LOCATION_MAP.get(src_id, src_id)
                if src_id not in outlets:
                    outlets[src_id] = make_outlet(src_id, sensor_location)
                    last_seen[src_id] = 0.0
                    n_samples[src_id] = 0
                    last_report_n[src_id] = 0
                    print(f"[NEW] LSL stream created: {LSL_PREFIX}_{sensor_location}")
